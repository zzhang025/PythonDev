...
class BankBranch:
    ...
    def open_account(self, customer_name):
        if len(self._tellers) == 0:
            raise Exception('No teller available')
    ...
    def withdraw(self, customer_id, amount):
        ...
        if amount > self._cash_on_hand:
            raise Exception('Insufficient cash on hand')
...
class Bank:
    ...
    def add_branch(self, address, initial_cash):
        branch = BankBranch(address, initial_cash, self._bank_system)
    ...
    def print_transactions(self):
        for transaction in self._bank_system.get_tranasactions():
